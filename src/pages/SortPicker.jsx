...
export default class SortPicker extends Component {
  ...
  setNewGirls(girls, pairCount) {
    ...
    this.setState({
      sortedGirls: girls,
      progress: Math.floor(
        (this.state.nextGirl / this.props.girls.length) * 100
      ),
      pairCount: pairCount,
      girl1: midGirl.girl,
    ...
    if (this.state.nextGirl > this.props.girls.length) {
      ...
      if (this.state.sortedGirls.length === 0) {
        firstPick = (
          <div>
            <VotingButton
              content={`No Opinion of ${this.state.girl1.name}`}
              className="sort-other-btn"
              onClick={() => this.onClick(NONE_FIRST)}
            />
          </div>
        );
      }
      ...
      sortPicker = (
        <div className="sort-picker">
          ...
          <div className="flex flex-center">
            <VotingButton
              content={girl1}
              className="sort-girl-btn"
              onClick={() => this.onClick(GIRL1)}
            />
            <p>vs</p>
            <VotingButton
              content={girl2}
              className="sort-girl-btn"
              onClick={() => this.onClick(GIRL2)}
            />
          </div>
          <br />
          <div>
            <VotingButton
              content="Tied"
              className="sort-other-btn"
              onClick={() => this.onClick(TIED)}
            />
          </div>
          {firstPick}
          <div>
            <VotingButton
              content={`No Opinion of ${this.state.girl2.name}`}
              className="sort-other-btn"
